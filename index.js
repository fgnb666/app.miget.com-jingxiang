...
const UUID = process.env.UUID || 'c171b0d2-5134-4c44-9fac-0adfeca936ac'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
const NEZHA_KEY = process.env.NEZHA_KEY || 'UsC5sNDciSaNggxJdB3EGSGYS242VT7q';              // 哪吒v1的NZ_CLIENT_SECRET或哪吒v0的agent密钥
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'miget.yszy.nyc.mn';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiZjUxNzk5OTMxMWQ4ZTAzOTg4OThmZjRkMTZiZDYzYjQiLCJ0IjoiYWE1MGU5NGItODViZS00M2Y4LWFjZTUtMTk2MTg0NDlmZTQxIiwicyI6Ik1HUTJaamcxWVdFdFpqSXhNQzAwT0dFeExUa3dZV1V0TVdZNU5tVTFNbVUzTm1VMSJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
const ARGO_PORT = process.env.ARGO_PORT || 43871;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
